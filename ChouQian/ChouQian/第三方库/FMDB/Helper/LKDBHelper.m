...
- (id)init
{
    //  chg by zqb 2013.12.18
//    return [self initWithDBName:@"ChatModel"];
    return [self initWithDBName:@"ShenghuoZhuShou"];
}
-(void)setDBName:(NSString *)fileName
{
    if([self.dbname isEqualToString:fileName] == NO)
    {
        ...
        //  chg by zqb 2013.10.28
        
        NSString * path = nil;
        if ([fileName hasPrefix:@"/Users/"]) {
            path = fileName;
        }
        else{
          path  = [[NSBundle mainBundle] pathForResource:self.dbname ofType:nil];
        }
        
        

        self.bindingQueue = [[FMDatabaseQueue alloc]initWithPath:path];
//        NSString *string = [XFTMyInfoManager myDBPath];
//        self.bindingQueue = [[FMDatabaseQueue alloc]initWithPath:[LKDBUtils getPathForDocuments:self.dbname inDir:string]];
...
-(BOOL)dropTableWithClass:(Class)modelClass tableName:(NSString*)tableName
{
...
-(BOOL)createTableWithModelClass:(Class)modelClass tableName:(NSString *)tableName
{
//    checkClassIsInvalid(modelClass);
...
//    NSString* tableName = nil;
    ...
    if(oldVersion>0 && oldVersion != newVersion)
    {
        ...
        switch (userOperation) {
            ...
            {
                [self dropTableWithClass:modelClass tableName:tableName];
            }
                break;
...
-(int)rowCount:(Class)modelClass where:(id)where tableName:(NSString*)tableName
{
    return [self rowCountBase:modelClass where:where tableName:tableName];
}
-(void)rowCount:(Class)modelClass where:(id)where callback:(void (^)(int))callback tableName:(NSString*)tableName

{
    [self asyncBlock:^{
        int result = [self rowCountBase:modelClass where:where tableName:tableName];
        if(callback != nil)
        {
            callback(result);
        }
...
-(int)rowCountBase:(Class)modelClass where:(id)where tableName:(NSString*)tableName
{
...
-(NSMutableArray *)search:(Class)modelClass where:(id)where orderBy:(NSString *)orderBy offset:(int)offset count:(int)count tableName:(NSString*)tableName
{
    return [self searchBase:modelClass where:where orderBy:orderBy offset:offset count:count tableName:tableName];
}
-(id)searchSingle:(Class)modelClass where:(id)where orderBy:(NSString *)orderBy tableName:(NSString*)tableName
{
    NSMutableArray* array = [self searchBase:modelClass where:where orderBy:orderBy offset:0 count:1 tableName:tableName];
...
-(void)search:(Class)modelClass where:(id)where orderBy:(NSString *)orderBy offset:(int)offset count:(int)count callback:(void (^)(NSMutableArray *))block tableName:(NSString*)tableName
{
    [self asyncBlock:^{
        NSMutableArray* array = [self searchBase:modelClass where:where orderBy:orderBy offset:offset count:count tableName:tableName];
...
-(NSMutableArray *)searchBase:(Class)modelClass where:(id)where orderBy:(NSString *)orderBy offset:(int)offset count:(int)count tableName:(NSString*)tableName
{
...
{
    return [self insertBase:model tableName:tableName];
}
-(void)insertToDB:(NSObject *)model callback:(void (^)(BOOL))block tableName:(NSString*)tableName
{
    [self asyncBlock:^{
        BOOL result = [self insertBase:model tableName:tableName];
        if(block != nil)
        {
            block(result);
        }
...
{
    if([self isExistsModel:model tableName:tableName]==NO)
    {
        return [self insertToDB:model tableName:tableName];
    }
    return NO;
}
...
tableName:(NSString*)tableName
{
    [self asyncBlock:^{
        if(block != nil)
        {
            block([self insertWhenNotExists:model tableName:tableName]);
        }
        else
        {
            [self insertWhenNotExists:model tableName:tableName];
        }
    }];
}
-(BOOL)insertBase:(NSObject*)model tableName:(NSString*)tableName
{
...
{
    return [self updateToDBBase:model where:where tableName:tableName];
}
-(void)updateToDB:(NSObject *)model where:(id)where callback:(void (^)(BOOL))block tableName:(NSString*)tableName
{
    [self asyncBlock:^{
        BOOL result = [self updateToDBBase:model where:where tableName:tableName];
        if(block != nil)
            block(result);
    }];
}
-(BOOL)updateToDBBase:(NSObject *)model where:(id)where tableName:(NSString*)tableName
...
-(BOOL)updateToDB:(Class)modelClass set:(NSString *)sets where:(id)where tableName:(NSString*)tableName
...
-(void)deleteToDB:(NSObject *)model callback:(void (^)(BOOL))block tableName:(NSString*)tableName
{
    [self asyncBlock:^{
        BOOL isDeleted = [self deleteToDBBase:model tableName:tableName];
        if(block != nil)
            block(isDeleted);
    }];
}
...
-(BOOL)deleteWithClass:(Class)modelClass where:(id)where tableName:(NSString*)tableName
{
    return [self deleteWithClassBase:modelClass where:where tableName:tableName];
}
-(void)deleteWithClass:(Class)modelClass where:(id)where callback:(void (^)(BOOL))block tableName:(NSString*)tableName
{
    [self asyncBlock:^{
        BOOL isDeleted = [self deleteWithClassBase:modelClass where:where tableName:tableName];
        if (block != nil) {
            block(isDeleted);
        }
    }];
}
-(BOOL)deleteWithClassBase:(Class)modelClass where:(id)where tableName:(NSString*)tableName
...
-(BOOL)isExistsModel:(NSObject *)model tableName:(NSString*)tableName
{
//    checkModelIsInvalid(model);
    if(model.rowid>0)
        return YES;
    else
    {
        ...
        if(primarykey&&primaryValue)
        {
            ...
            return [self isExistsClass:modelClass where:where tableName:tableName];
        }
...
-(BOOL)isExistsClass:(Class)modelClass where:(id)where tableName:(NSString*)tableName
{
    return [self isExistsClassBase:modelClass where:where tableName:tableName];
}
-(BOOL)isExistsClassBase:(Class)modelClass where:(id)where tableName:(NSString*)tableName
{
    return [self rowCount:modelClass where:where tableName:tableName] > 0;
}

#pragma mark- clear operation

+(void)clearTableData:(Class)modelClass tableName:(NSString*)tableName
...
+(void)clearNoneImage:(Class)modelClass columes:(NSArray *)columes tableName:(NSString*)tableName
{
    [self clearFileWithTable:modelClass columes:columes type:1 tableName:tableName];
}
+(void)clearNoneData:(Class)modelClass columes:(NSArray *)columes tableName:(NSString*)tableName
{
    [self clearFileWithTable:modelClass columes:columes type:2 tableName:tableName];
}
#define LKTestDirFilename @"LKTestDirFilename111"
+(void)clearFileWithTable:(Class)modelClass columes:(NSArray*)columes type:(int)type tableName:(NSString*)tableName
...
/*作者:窦静轩    描述:找到where 单个值的*/

#pragma mark 找到where 单个值的
-(id)getTableSingleValueBySQLFunc:(NSString*)sqlWhere tableName:(NSString*)tableName modelClass:(Class)modelClass
{
    NSMutableString* query = [NSMutableString stringWithFormat:@"%@ from %@",sqlWhere,tableName];
