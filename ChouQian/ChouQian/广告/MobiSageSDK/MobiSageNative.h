...
-(id) initWithWidth:(CGFloat) width completion:(MobiSageNativeCompletionCallBack)completion;

//请求广告资源.(height=期望广告view的宽度.我们会根据height,等比缩放广告view,并返回给你width).
-(id) initWithHeight:(CGFloat) height completion:(MobiSageNativeCompletionCallBack)completion;

//请求广告资源.(size=期望广告view的size.我们会自适应填充广告内容.宽高比在0.5-2之间为有效size).
-(id) initWithSize:(CGSize) size completion:(MobiSageNativeCompletionCallBack)completion;
